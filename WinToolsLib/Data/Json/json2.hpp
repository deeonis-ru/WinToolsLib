...
void basic_json<Char>::to_stream(basic_json_output_handler<Char>& handler) const
{
    ...
    case object_t:
        ...
        for (const_object_order_iterator it = o->begin_order(); it != o->end_order(); ++it)
        {
            handler.name(json_object<Char>::get_name(it));
            json_object<Char>::get_basic_json(it).to_stream(handler);
        }
        handler.end_object();
        }
        break;
